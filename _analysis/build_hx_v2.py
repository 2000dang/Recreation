...
data['system_prompt'] = (data.get('system_prompt','') +
    '\n11. **变量更新格式·强制包裹**: 所有变量更新的内容（包括分析文本+JSON数组）必须包裹在 `<UpdateVariable>...</UpdateVariable>` 之间。'
    '绝对禁止在消息正文中裸露输出变量解析的 JSON 数组或分析文本（如数字编号 "1. 2. 3." 列表、`○` 项目符号、裸 `[{...}]` 数组）。'
    '任何裸变量的输出必须被<UpdateVariable>标签包裹——AI在最终生成回复前必须在内部检查是否有裸 JSON 或分析列表，'
    '有则强制用 <UpdateVariable> 包裹。违者视为严重格式错误。'
    ...
    '\n13. **变量输出后清理**: 在最终生成回复时，AI必须检查消息末尾是否存在未被<UpdateVariable>包裹的裸 JSON 数组（以`[{"op"`开头的部分）。'
    '如果存在，必须用<UpdateVariable>标签将该数组包裹起来。这是最终的下限检查，不可跳过。')
...
CONTENT = [
 ...
 ('杨玉兰', ['杨玉兰','小杨','实习助理','玉兰'],
  ...
  '在办公室时背对{{user}}站立办公（{{user}}在她的正后方，对着她的背影和后脑勺），不许回头。'),
 ...
 ('助理管理·职级与编制', ['助理编制','助理配额','职级与编制','管理职数','女助理晋升','助理竞聘','女助理等级'],
  ...
  '·实习助理：蓝色OL制服+白丝袜+高跟，2万月薪，站立工作12小时，背对{{user}}（{{user}}在前方面向助理后背），不许回头\n'
