...
(function () {
  ...
  function getHackMode() {
    try {
      var sd = getStatData();
      if (sd && sd.主角状态 && typeof sd.主角状态.破解模式 === 'boolean') return !!sd.主角状态.破解模式;
    } catch(e){}
    try { return localStorage.getItem(HX_CONFIG.hack) === '1'; } catch(e){}
    return false;
  }
  function setHackMode(on) {
    ...
    try { writeBackMvu(function(sd) {
      ...
      if (next) {
        // 备份当前值
        if (typeof p.贡献点 === 'number' && p.贡献点 >= 0) sd.__hx_backup_contrib = p.贡献点;
        else if (typeof sd.__hx_backup_contrib !== 'number') sd.__hx_backup_contrib = 5;
        if (typeof p.催眠权限等级 === 'number') sd.__hx_backup_hypnosis_level = p.催眠权限等级;
        if (typeof p.精神状态 === 'number') sd.__hx_backup_mental = p.精神状态;
        p.贡献点 = 999999;   // 显示为∞由渲染逻辑处理
        p.催眠权限等级 = 10;
        p.精神状态 = 100;
        p.破解模式 = true;
      } else {
        p.贡献点 = (typeof sd.__hx_backup_contrib === 'number') ? sd.__hx_backup_contrib : 5;
    ...
    }); } catch(e){ console.warn('[环晓] setHackMode write failed', e); }
  }
